...
```text
mode:=sim   启动 Gazebo + 机器人 + RViz
mode:=slam  启动 Gazebo + 机器人 + Cartographer + SLAM RViz
mode:=nav   启动 Gazebo + 机器人 + AMCL + Nav2 + RViz
```

如果 `mode:=nav` 指定的地图不存在，launch 会提示先建图并保存地图，然后退出。
